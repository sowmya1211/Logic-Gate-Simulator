//NOR gate- Derived class of Gate 
package gates;
import exceptions.*;

public class NOR<T extends Comparable> extends Gates
{
    //Arrays to check Input, Output Pin Combinations
    int inputCombos[] = {0,3,1,1,3,1,1,0,1,1,3,1,1,3,0};
    int outputCombos[] = {0,1,1,1,4,2,2,0,3,3,9,4,4,16,0};

    public NOR(int VCC, int gnd, int inputPinNo1,int inputPinNo2, int outputPinNo, T input1, T input2)
    {
    ...
    public String assignOutput()
    {
         ...
         {
          if ( input1.compareTo("0")==0 && input2.compareTo("0")==0 )
             output=1;
          else
             output=0;   
         }  
